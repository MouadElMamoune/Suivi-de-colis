...
namespace Suivi_de_colis
{
    class DeplacementDAO
    {
        ...
        {
            var requete = client.Cypher.Match("(c:Camion)", "(s:Destination)").Where("c.ID = '" + C.ID + "'").AndWhere("s.ID = '" + S.ID + "'").Create("(c)-[dep:de {ID :'" + dep.ID + "', Date_de_depart : '" + dep.Date_de_depart + "', Date_arrive : '" + dep.Date_arrive + "'}]->(s)").ExecuteWithoutResultsAsync();
            requete.Wait();
            requete = client.Cypher.Match("(c:Camion)", "(d:Destination)").Where("c.ID = '" + C.ID + "'").AndWhere("d.ID = '" + D.ID + "'").Create("(c)-[dep:à {ID :'" + dep.ID + "', Date_de_depart : '" + dep.Date_de_depart + "', Date_arrive : '" + dep.Date_arrive + "'}]->(d)").ExecuteWithoutResultsAsync();
            requete.Wait();
        }

        public void Supprimer(Deplacement dep, Camion C, Destination S, Destination D)
        {
            var requete = client.Cypher.Match("(c:Camion { ID : '" + C.ID + "' })-[dep:de]->(s:Destination { ID : '" + S.ID + "' })").Delete("dep").ExecuteWithoutResultsAsync();
            requete.Wait();
